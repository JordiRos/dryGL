...
{
    bool res = false;
    dry::Log(LogSystem, "[ImageLoader] Load texture from file: %s", file.c_str());
...
{
    bool res = false;
    dry::Log(LogSystem, "[ImageLoader] Load image from file: %s", file.c_str());
    NSString *path  = [NSString stringWithUTF8String:file.c_str()];
    NSData   *data  = [[NSData alloc] initWithContentsOfFile:path];
    UIImage  *image = [[UIImage alloc] initWithData:data];
    if (image != nil)
